...
def app():
    ...
    if st.button("Get how much to order"):
        # Make request
        send_request("http://localhost:5000/api/how_much_to_order", payload)

    if st.button("Get stock level forecast"):
        # Make request
        send_request("http://localhost:5000/api/stock_level_forecast", payload)

    if st.button("Get low stock sku list"):
        # Make request
        send_request("http://localhost:5000/api/low_stock_sku_list", payload_list)
